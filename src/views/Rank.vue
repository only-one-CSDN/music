        ...
        <ul>
          <li v-for="(value, key) in category.titles" :key="key">
            ...
            <ul class="normal-group" v-if="value === '官方榜'" >
              <li v-for="(obj, index) in category[key]" :key="index" @click.stop="selectedItem(obj.id)">
                <div class="rank-left">
                  <img v-lazy="obj.rank.coverImgUrl" alt="">
                  <p>{{obj.rank.updateFrequency}}</p>
                </div>
                <div class="rank-right">
                  <p v-for="(song, index) in obj.rank.tracks" :key="index"> {{index}}.{{song.first}}-{{song.second}}</p>
                </div>
              </li>
            </ul>
